...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "`/market` — All indices\n"
        "`/briefing` — Subscribe to 9 AM briefing\n",
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "nifty":                       await nifty_cmd(update, context)
        ...
        await q.message.reply_text("📨 Building your briefing...", parse_mode="Markdown")
        text = await build_morning_briefing()
        await q.message.reply_text(text, parse_mode="Markdown")
    elif d == "briefing_unsub":
        briefing_subscribers.pop(q.from_user.id, None)
        await q.edit_message_text("❌ Unsubscribed from morning briefings.\n\nUse /briefing to subscribe again.")
    elif d == "nifty_ai":
        nd = get_full_analysis(NIFTY_TICKERS)
        if nd:
            r = client.messages.create(
...
def main():
    print("🚀 Starting Advanced Trading Bot v3 + Morning Briefing...")
    ...
    print("✅ Bot v3 running! Morning briefing scheduler active.")
