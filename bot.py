...
def fetch_data(tickers, period="3mo", interval="1d"):
    """
    6-source waterfall — nsepython first (free, HTTPS, works on Railway):
    0) nsepython  — live NSE official data, HTTPS port 443, no account
    1) NSE API    — direct NSE REST, zero delay
    2) Stooq      — reliable cloud-friendly daily data
    3) yfinance   — standard library
    4) Yahoo direct API
    ...
    if interval == "1d" and NSE_PYTHON_OK:
        ...
        if live and live["price"] > 0:
            # Get historical from Stooq, patch last price with live NSE data
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🤖 *AI Trading Bot v9 — Smart Signals*\n\n"
        "🧠 *Professional Strategies:*\n"
        "`/strategy` or `/st` — All 5 strategies + best signal\n"
        "`/signal` — Today's signal with Entry/SL/Targets NOW\n"
        "`/regime` — Market regime detection\n"
        "`/performance` — Strategy win rates\n"
        "`/learn` — AI self-improvement\n\n"
        "📊 *Analysis:*\n"
        ...
        "🔔 *Auto Alerts (subscribe with /briefing):*\n"
        "• 9:00 AM — Morning market briefing\n"
        "• 9:30 AM — 🆕 Auto signal with Entry/SL/Targets\n"
        "• 3:30 PM — Closing summary\n"
        "• Real-time volatility alerts\n\n"
        "📸 *Send any chart — 3-pass AI analysis!*\n"
        "_Identifies patterns, indicators + exact trade setup_\n",
...
async def handle_chart_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Pass 1: Identify chart type, instrument, timeframe
    Pass 2: Deep technical analysis with exact price levels
    Pass 3: Generate precise trade setup with SL/Target
    """
    msg = await update.message.reply_text(
...
def main():
    ...
    commands = [
        ...
        ("signal",      signal_cmd),     # Manual signal trigger
    ]
