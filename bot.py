...
def fetch_data(tickers, period="3mo", interval="1d"):
    """
    5-source waterfall:
    1) NSE Official API (real-time, zero delay) — Indian indices only
    2) Stooq (reliable from cloud, daily)
    3) yfinance library
    4) Yahoo Finance direct API
    5) Investing.com scrape (last resort)
    """
    # Source 1: NSE Official — real-time, only for daily/current price
    if interval == "1d" and tickers[0] in ("^NSEI","^NSEBANK","NIFTY50.NS","BANKNIFTY.NS"):
        nse_data = fetch_nse_official(tickers[0])
        if nse_data and nse_data["price"] > 0:
            logger.info(f"NSE official OK: {tickers[0]} @ {nse_data['price']}")
            # Build minimal DataFrame for compatibility, real-time price patched in
            df_hist, _ = fetch_stooq(tickers[0], period, interval), None
            if isinstance(df_hist, pd.DataFrame) and not df_hist.empty:
                # Update last row with real-time NSE price
    ...
    # Source 5: Investing.com (last resort, daily only)
    if interval == "1d":
        df = fetch_investing_fallback(tickers[0])
        if df is not None:
            return df, tickers[0]
...
try:
    ...
    USE_PANDAS_TA = True
    logger.info("pandas-ta loaded — using accurate indicators")
except ImportError:
    USE_PANDAS_TA = False
    logger.warning("pandas-ta not installed — using fallback indicators")

def rsi_series(s, n=14):
    """Fix 2a: RSI using Wilder smoothing (correct method, not simple rolling)."""
    if USE_PANDAS_TA:
        result = ta.rsi(s, length=n)
        return result.fillna(50) if result is not None else _rsi_fallback(s, n)
    return _rsi_fallback(s, n)

def _rsi_fallback(s, n=14):
    """Wilder smoothing RSI — more accurate than simple rolling average."""
    delta = s.diff()
...
def calc_atr(h, l, c, n=14):
    """Fix 2b: Wilder ATR — the correct industry-standard calculation."""
    if USE_PANDAS_TA:
        result = ta.atr(h, l, c, length=n)
        if result is not None and not result.empty:
            return result.fillna(method="bfill")
    # Wilder smoothing ATR fallback
    tr = pd.concat([
        (h - l),
        (h - c.shift()).abs(),
        (l - c.shift()).abs()
...
def calc_macd(s, fast=12, slow=26, sig=9):
    """Fix 2c: MACD with proper EMA calculation."""
    if USE_PANDAS_TA:
        result = ta.macd(s, fast=fast, slow=slow, signal=sig)
        if result is not None and not result.empty:
            cols = result.columns.tolist()
            m  = round(float(result[cols[0]].iloc[-1]), 2)
            sg = round(float(result[cols[2]].iloc[-1]), 2)
            h  = round(float(result[cols[1]].iloc[-1]), 2)
            return m, sg, h
...
def calc_bb(s, n=20, std=2):
    """Bollinger Bands."""
    if USE_PANDAS_TA:
        result = ta.bbands(s, length=n, std=std)
        if result is not None and not result.empty:
            cols = result.columns.tolist()
            return (round(float(result[cols[2]].iloc[-1]),2),
                    round(float(result[cols[1]].iloc[-1]),2),
                    round(float(result[cols[0]].iloc[-1]),2))
    sm = s.rolling(n).mean()
    sd = s.rolling(n).std()
...
def calc_adx(h, l, c, n=14) -> float:
    """Fix 2d: ADX — trend strength filter. Above 25 = trending, below 20 = choppy."""
    if USE_PANDAS_TA:
        result = ta.adx(h, l, c, length=n)
        if result is not None and not result.empty:
            adx_col = [col for col in result.columns if col.startswith("ADX")]
            if adx_col:
                return round(float(result[adx_col[0]].iloc[-1]), 2)
    # Manual ADX fallback
    try:
...
def main():
    print("🚀 Starting Advanced Trading Bot v7 — Strategy Engine Active...")
    ...
    # Initialise persistent database + strategy tables
    db_init()
    db_init_strategies()
